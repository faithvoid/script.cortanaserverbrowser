...
NOTIFY_XLINK = os.path.join(SCRIPT_DIR, "notify-xlink.py")
XLINK_NEWS = os.path.join(SCRIPT_DIR, "xlink-news.py")
    ...
    for item in channel.findall('item'):
        ...
        if title_elem is not None:
            ...
            if any(prefix in title_text for prefix in ["Users", "Server Status", "Supported Games", "Total Users", "Orbital", "Game Traffic"]):
                ...
                stats.append(title_elem.text.replace("&amp;", "&"))

    selected = dialog.select("Statistics - XLink Kai", stats)
...
def display_events(dialog, channel, args=None):
    ...
    for item in channel.findall('item'):
        ...
        if title_elem is not None and title_elem.text.startswith("Game Event"):
            description = clean_html(description_elem.text) if description_elem is not None else "No description available"
            event_text = title_elem.text.replace("&amp;", "&").replace("Game Event - ", "") + " - " + description
...
def display_xlink_notifications(dialog, channel):
    xbmc.executebuiltin('RunScript("{}")'.format(NOTIFY_XLINK))

# Function to display news
def xlink_news(dialog, channel):
    xbmc.executebuiltin('RunScript("{}")'.format(XLINK_NEWS))
...
def main():
    ...
    feeds = [
        ...
        ("Enable | Disable Notifications", display_xlink_notifications),
        ("News", xlink_news),
    ]

    if len(sys.argv) > 1:
        arg_name = sys.argv[1].capitalize()
        for name, func in feeds:
            if name == arg_name:
                url = "http://ogxbox.org/rss/xlinkkai"
                if name == "Sessions":
                    url += "?matchmakingonly&noevents"
    ...
    if selected >= 0:
        name, selected_function = feeds[selected]

        # Determine the appropriate URL based on selection
        url = "http://ogxbox.org/rss/xlinkkai"
        if name == "Sessions":
            url += "?matchmakingonly&noevents"
        elif name == "Statistics":
