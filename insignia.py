...
def display_stats(dialog, channel, args=None):
    ...
    for item in channel.findall('item'):
        ...
        if title_elem is not None and any(prefix in title_elem.text for prefix in ["Users Online", "Registered Users", "Games Supported", "Active Games"]):
            stats.append(title_elem.text.replace("&amp;", "&").strip())
    selected = dialog.select("Statistics - Insignia", stats)
    if selected == 0:
        main()
    elif selected > 0:
...
def display_feed_items(dialog, channel, args=None):
    ...
    for item in channel.findall('item'):
        ...
        if title_elem is not None:
            title = title_elem.text.replace("&amp;", "&").strip()  # Remove leading/trailing whitespace from title
...
def display_insignia_notifications(dialog, channel):
    xbmc.executebuiltin('RunScript("{}")'.format(NOTIFY_INSIGNIA))

def display_leaderboard(dialog, channel):
    xbmc.executebuiltin('RunScript("{}")'.format(DISPLAY_LEADERBOARD))
...
#        ("Scoreboard", display_leaderboard),
    ...
    if len(sys.argv) > 1:
        ...
        for name, func in feeds:
            if name == arg_name:
                url = "http://ogxbox.org/rss/insignia"
                if name == "Sessions":
